...
bool CSpectatorList::GetSpectators()
{
	...
	for (int n = 1; n < I::EngineClient->GetMaxClients() + 1; n++)
	{
		...
		if (nMode != OBS_MODE_FIRSTPERSON && nMode != OBS_MODE_THIRDPERSON)
			continue;
		...
		m_vecSpectators.emplace_back(Spectator_t{ Util::ConvertUtf8ToWide(PlayerInfo.name), nMode });
	}
...
void CSpectatorList::Run()
{
	...
	if (!F::Menu.IsOpen() && I::EngineVGui->IsGameUIVisible())
		return;
	...
	for (size_t n = 0; n < m_vecSpectators.size(); n++)
	{
		...
		H::Draw.String(
			...
			ALIGN_CENTER,
			Spectator.m_nMode == OBS_MODE_FIRSTPERSON ? "1st" : "3rd"
		);
