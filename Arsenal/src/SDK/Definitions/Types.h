...
{
	matrix3x4_t() {}
	matrix3x4_t(vec_t m00, vec_t m01, vec_t m02, vec_t m03, vec_t m10, vec_t m11, vec_t m12, vec_t m13, vec_t m20, vec_t m21, vec_t m22, vec_t m23)
	{
	...
	vec_t* operator[](int i) { return m_flMatVal[i]; }
	const vec_t* operator[](int i) const { return m_flMatVal[i]; }

	vec_t m_flMatVal[3][4];
};
...
{
	Vertex_t() {}
	Vertex_t(const Vec2& pos, const Vec2& coord = Vec2(0, 0)) { m_Position = pos; m_TexCoord = coord; }
...
public:
	...
	inline void Init(float ix = 0.f, float iy = 0.f, float iz = 0.f) { x = ix; y = iy; z = iz; }
...
struct Color_t
{
	...
	inline Color_t Lerp(Color_t to, float t, LerpEnum::LerpEnum eLerp = LerpEnum::All) const
	{
		...
		case LerpEnum::HSVNoAlpha:
		{
			...
			Color_t tOut; tOut.SetHSV(fmodf(Math::LerpAngle(flHFrom - 180, flHTo - 180, t) + 180, 360), Math::Lerp(flSFrom, flSTo, t), Math::Lerp(flVFrom, flVTo, t), flAOut);
			return tOut;
		}
		}
	}
