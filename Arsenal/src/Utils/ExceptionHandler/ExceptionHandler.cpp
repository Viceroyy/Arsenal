...
static LONG APIENTRY ExceptionFilter(PEXCEPTION_POINTERS ExceptionInfo)
{
	...
	{
		ssErrorStream << std::format("Exception at: {:#x}", reinterpret_cast<uintptr_t>(ExceptionInfo->ExceptionRecord->ExceptionAddress));
		ssErrorStream << "\n";
	}
	...
	{
		std::ofstream file;
		file.open("Arsenal\\crash_log.txt", std::ios_base::app);
		file << ssErrorStream.str() + "\n\n\n";
		file.close();

		ssErrorStream << "\n";
		ssErrorStream << "Ctrl + C to copy. \n";
		ssErrorStream << "Logged to Arsenal\\crash_log.txt. ";
	}
	catch (...) {}
	...
	case STATUS_HEAP_CORRUPTION:
		MessageBox(0, ssErrorStream.str().c_str(), "Unhandled exception", MB_OK | MB_ICONERROR);
	}
